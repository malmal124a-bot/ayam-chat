...
class _InventoryScreenState extends State<InventoryScreen> {
  ...
  void _equipItem(StoreItem item) {
    switch (item.type) {
      ...
      case StoreItemType.fancyId:
        // For Fancy ID, equipping means setting it as the user's equipped vanity ID
        bool isCurrentlyActive = userController.displayId == item.name;
        if (!isCurrentlyActive) {
          userController.setFancyId(item.name);
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('تم تفعيل الآي دي المميز: ${item.name}')),
          );
        } else {
          // Unequip the vanity ID by setting it to empty
          userController.setFancyId('');
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('تم إلغاء تفعيل الآي دي المميز')),
          );
        }
        break;
    }
  ...
  Widget _buildInventoryItemCard(StoreItem item, ThemeData theme) {
    ...
    if (item.type == StoreItemType.frame) {
      ...
      isActive = userController.displayId == item.name;
    }
