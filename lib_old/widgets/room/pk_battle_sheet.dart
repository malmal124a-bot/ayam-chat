...
class PKBattleSheet extends StatefulWidget {
  ...
  const PKBattleSheet({super.key, required this.roomId, this.onStartPK});
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: _isStarting ? null : () async {
                    ...
                    try {
                      // Create PK battle document in Firestore
                      final pkBattleData = {
                        'mode': selectedMode,
                        'durationMinutes': selectedDuration,
                        'startTime': FieldValue.serverTimestamp(),
                        'endTime': DateTime.now().add(Duration(minutes: selectedDuration)),
                        'isActive': true,
                        'redTeamScore': 0,
                        'blueTeamScore': 0,
                      ...
                      await _firestore.collection('pk_battles').add(pkBattleData);
                      
                      // Update room with PK battle state
                      await _firestore.collection('rooms').doc(widget.roomId).update({
                        'pkBattleActive': true,
                        'pkBattleMode': selectedMode,
                        'pkBattleDuration': selectedDuration,
                        'pkBattleStartTime': FieldValue.serverTimestamp(),
                      });
                      ...
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('فشل بدء التحدي، حاول مرة أخرى')),
                        );
                      }
                    } finally {
                      if (mounted) {
