...
class RoomUserProfileSheet extends StatelessWidget {
  ...
  void _openGiftSheet(BuildContext context, RoomUiController roomController) {
    ...
    showComprehensiveGiftSheet(
      ...
      giftController, 
      (msg, target, combo) {
        roomController.addMessage(RoomMessage(
          id: DateTime.now().millisecondsSinceEpoch.toString(),
          senderName: userController.name,
          text: "أرسل $msg x$combo",
          type: RoomMessageType.gift,
          senderLevel: userController.currentLevel,
        ));
      }, 
  ...
  Widget build(BuildContext context) {
    ...
    final bool canManage = isCurrentUserOwner || isCurrentUserAdmin;
    final bool isMe = seat.userId == userController.id;
    ...
    return Row(
      ...
      children: [
        _actionItem(
          ...
          Colors.redAccent, 
          () async {
            await controller.kickUserFromSeat(seat.index);
            if (context.mounted) {
        ...
        Wrap(
          ...
          children: [
            _actionItem(
              ...
              Colors.blue, 
              () async {
                await controller.toggleSeatLock(seat.index);
                if (context.mounted) {
            ...
            _actionItem(
              ...
              Colors.orange, 
              () async {
                await controller.toggleSeatMute(seat.index);
                if (context.mounted) {
            ...
            _actionItem(
              ...
              Colors.deepOrange, 
              () async {
                await controller.kickUserToAudience(seat.index);
                if (context.mounted) {
                  Navigator.pop(context);
            ...
            _actionItem(
              ...
              Colors.red, 
              () async {
                await controller.banUserFromSeat(seat.index);
                if (context.mounted) {
                  Navigator.pop(context);
    ...
    return Row(
      ...
      children: [
        _actionItem(
          ...
          () async {
            await controller.toggleFollow(seat.userId ?? '');
            if (context.mounted) {
