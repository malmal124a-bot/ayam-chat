...
class AgencyController extends ChangeNotifier {
  ...
  void _listenToFirestore() {
    ...
    _firestore.collection('agencies')
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        _myOwnedAgency = _parseAgencyFromFirestore(snapshot.docs.first.id, data);
        notifyListeners();
      }
    });

    // Listen to user's joined agency
    _firestore.collection('users').doc(userId).snapshots().listen((snapshot) {
      if (snapshot.exists) {
        final data = snapshot.data();
        if (data != null && data['agencyId'] != null) {
          _firestore.collection('agencies').doc(data['agencyId']).get().then((doc) {
            if (doc.exists) {
              _joinedAgency = _parseAgencyFromFirestore(doc.id, doc.data()!);
              notifyListeners();
            }
          });
        }
  ...
  Agency _parseAgencyFromFirestore(String id, Map<String, dynamic> data) {
    // Parse agency data from Firestore document
    return Agency(
      id: id,
      name: data['name'] ?? '',
      ...
      agencyType: AgencyType.values.firstWhere(
        (e) => e.toString() == data['agencyType'],
        orElse: () => AgencyType.modife,
      ),
      members: [], // Members would need to be loaded separately
  ...
  // Earnings storage for hosts not in agencies
  ...
  Map<String, dynamic> swapDiamonds(int amount) {
    ...
    _saveAgencyTransaction(user.id, amount, 'swap_diamonds');

    return {
      'ok': true,
      'message': 'تم تبديل $amount ماسة بنجاح',
    };
  }

  Future<void> _saveAgencyTransaction(String userId, int amount, String type) async {
    try {
      await _firestore.collection('users').doc(userId).collection('agency_transactions').add({
        'amount': amount,
        'type': type,
        'timestamp': FieldValue.serverTimestamp(),
      });
    } catch (e) {
      debugPrint('Error saving agency transaction: $e');
    }
  }
  ...
  Future<Map<String, dynamic>> sellTargetToAgent(double amount) async {
    ...
    wallet.addAgencyBalance(usdValue);
    ...
    if (_myOwnedAgency != null) {
      ...
      _myOwnedAgency!.chargingLogs.insert(0, transaction);
