...
export default function CPPage() {
  ...
  useEffect(() => {
    ...
    getAppConfig().then(cfg => {
      if ((cfg as any)?.cpWebUrl) setCpWebUrl((cfg as any).cpWebUrl as string);
      ...
      <div>
        ...
        <DataTable
          ...
          columns={[
            ...
            { key: 'revenueShare', label: 'Revenue Share', sortable: true, render: (c: CPModel) => <span>{c.revenueShare}%</span> },
            { key: 'contentCount', label: 'Content Count', sortable: true },
            { key: 'status', label: 'Status', sortable: true, render: (c: CPModel) => <span className={c.status === 'active' ? 'text-emerald-400' : 'text-rose-400'}>{c.status}</span> },
          ]}
          data={cps}
