...
export default function BadgesPage() {
  ...
  const handleSave = async () => {
    if (!editing) return;
    try {
      await updateBadge(editing.id, { ...form, iconAsset: form.iconAsset || null, svgaUrl: form.svgaUrl || null } as Partial<BadgeConfig>);
      setEditing(null);
      resetForm();
      load();
    } catch (e) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await addBadge(id, { ...form, id, iconAsset: form.iconAsset || null, svgaUrl: form.svgaUrl || null } as BadgeConfig);
      setShowAdd(false);
      resetForm();
      load();
    } catch (e) {
        ...
        <div className="bg-[#141417] rounded-2xl border border-indigo-500/20 p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <ImageUpload
              ...
              label="📺 شارة SVGA / Animate (الأساسي)"
              accept=".svga,.vap,.apng,.pag,.json,.webp,.png"
            />
            <ImageUpload
              currentUrl={form.imageUrl}
              onUpload={file => uploadBadgeIcon(file, form.id || 'new_' + Date.now())}
              onUrlChange={url => updateField('imageUrl', url)}
              label="🖼️ صورة معاينة (PNG/WebP/APNG)"
