...
export default function CpFeaturesPage() {
  ...
  const [rewardForm, setRewardForm] = useState({ period: 'daily', rank_position: 1, sort_order: 0, reward_type: 'frame_svga', label_ar: '', label_en: '', svga_url: '', image_url: '' });
  const [editingReward, setEditingReward] = useState<number | null>(null);
  const [showAddReward, setShowAddReward] = useState(false);
  const [rewardsPeriod, setRewardsPeriod] = useState<'all' | 'daily' | 'weekly' | 'monthly'>('all');
  ...
  const loadGifts = () => getCpGifts().then(setGifts);
  const loadCars = () => getCpCars().then(setCars);
  const loadRewards = () => getCpRankRewards().then(setRewards);
  ...
  const handleSaveSettings = async () => {
    ...
    for (const [key, value] of Object.entries(cpSettings)) {
      await updateCpSetting(key, String(value));
    }
    setSavingSettings(false);
    showMsg('Settings saved!');
  };

  const handleImageUpload = async (file: File, key: string): Promise<string> => {
    try {
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            {[
              ...
              <button key={tab.key} onClick={() => setRewardsPeriod(tab.key as 'all' | 'daily' | 'weekly' | 'monthly')}
