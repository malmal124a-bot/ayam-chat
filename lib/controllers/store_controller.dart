...
class StoreController extends ChangeNotifier {
  ...
  /// Merges items managed from the admin dashboard (public.store_items)
  /// into the local catalog. Remote items override local ones with the same id.
  void mergeCatalog(List<Map<String, dynamic>> rows) {
    var changed = false;
    for (final row in rows) {
      final type = CatalogService.storeTypeFrom(row['item_type']?.toString());
      if (type == StoreItemType.fancyId) {
        continue; // fancy IDs stay local to avoid clashing with purchased ones
      }
      final id = row['id']?.toString();
      if (id == null || id.isEmpty) continue;
      final svgaUrl = row['svga_url']?.toString() ?? '';
      debugPrint('CatalogService: item=$id imagePath=${row['image_url']} svgaUrl=$svgaUrl');
      final remote = StoreItem(
        id: id,
        name: row['name']?.toString() ?? 'عنصر',
      ...
      if (index >= 0) {
        ...
        if (old.name != remote.name ||
            ...
            old.svgaUrl != remote.svgaUrl ||
            old.price != remote.price ||
            old.type != remote.type) {
          _items[index] = remote;
          changed = true;
        }
      } else {
