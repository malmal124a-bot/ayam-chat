        ...
        child: Column(
          children: [
            ...
            TabBar(
              ...
              indicator: BoxDecoration(color: AppTheme.royalGold.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(20)),
              tabs: const [
                Tab(text: 'يومي'),
                Tab(text: 'أسبوعي'),
                Tab(text: 'شهري'),
              ...
              builder: (context, controller, child) {
                return Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      _ResetInfo(label: 'يومي', time: 'ينتهي ${controller.getDailyResetTime()}', icon: Icons.access_time),
                      _ResetInfo(label: 'أسبوعي', time: 'يتصفّر ${controller.getWeeklyResetTime()}', icon: Icons.date_range),
                      _ResetInfo(label: 'شهري', time: 'يتصفّر ${controller.getMonthlyResetTime()}', icon: Icons.calendar_month),
                    ],
                  ),
                );
              },
...
class _ResetInfo extends StatelessWidget {
  ...
  final String time;
  final IconData icon;
  const _ResetInfo({required this.label, required this.time, required this.icon});

  @override
  Widget build(BuildContext context) {
    return Row(
      children: [
        Icon(icon, size: 12, color: AppTheme.royalGold),
        ...
        Column(
          ...
          children: [
            ...
            Text(time, style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 9)),
          ],
        ),
      ],
    );
...
class _PodiumBadge extends StatelessWidget {
  ...
  final int rank;
  final bool isMain;
  const _PodiumBadge({required this.user, required this.rank, required this.isMain});
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 12),
          CircleAvatar(radius: 22, backgroundImage: _getSafeProvider(user.avatarUrl), backgroundColor: Colors.grey),
          const SizedBox(width: 12),
          Expanded(
            child: Text(user.userName, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14), maxLines: 1, overflow: TextOverflow.ellipsis),
          ),
          Column(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              Text('${user.giftScore}', style: TextStyle(color: AppTheme.royalGold, fontWeight: FontWeight.bold, fontSize: 16)),
              Text('ماسة', style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 10)),
            ],
          ),
        ],
      ),
...
ImageProvider _getSafeProvider(String? path) {
  ...
  if (path == null || path.isEmpty) return const AssetImage(fallback);
