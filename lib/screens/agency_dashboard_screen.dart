...
class _AgencyDashboardScreenState extends State<AgencyDashboardScreen> {
  ...
  void _processCharge() async {
    ...
    try {
      ...
      try {
        ...
        if (wallet.agencyBalance < costUsd) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _handleQuickRecharge(int amount) {
    // Agency wallet top-up must go through the admin dashboard payment gateways
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        backgroundColor: Colors.orange,
        content: const Text('لشحن رصيد الوكالة، يرجى استخدام لوحة التحكم على الويب.'),
      ),
    );
  }
        ...
        child: SingleChildScrollView(
          ...
          child: ConstrainedBox(
            ...
            child: Column(
              ...
              children: [
                _buildBalanceCard(wallet.agencyBalance),
                const SizedBox(height: 30),
                Text('إعادة شحن سيولة سريعة', style: TextStyle(color: AppTheme.darkBrown, fontSize: 18, fontWeight: FontWeight.bold)),
                ...
                Text('شحن رصيد لعميل', style: TextStyle(color: AppTheme.darkBrown, fontSize: 18, fontWeight: FontWeight.bold)),
                const SizedBox(height: 15),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Text('\$$amount', style: TextStyle(color: AppTheme.royalGold, fontWeight: FontWeight.bold, fontSize: 18)),
  ...
  Widget _buildLogsHeader() {
    return Row(
      ...
      children: [
        Text('سجل معاملات الوكالة', style: TextStyle(color: AppTheme.darkBrown, fontSize: 18, fontWeight: FontWeight.bold)),
        TextButton(
          onPressed: () {},
          child: Text('عرض الكل', style: TextStyle(color: AppTheme.royalGold)),
        ),
      ],
    );
  }
  ...
  Widget _buildTextField(TextEditingController controller, String label, IconData icon, {TextInputType? keyboardType}) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: BorderSide(color: AppTheme.royalGold)),
      ),
    );
  }
}